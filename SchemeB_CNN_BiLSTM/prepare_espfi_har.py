...
def main() -> None:
    ...
    split_meta: dict[str, list[dict[str, int | str]]] = {"train": [], "val": [], "test": []}
    ...
    for split in ["train", "val", "test"]:
        summary["splits"][split] = {
            ...
            "windows": len(split_windows[split]),
            "label_counts": dict(sorted(Counter(split_labels[split]).items())),
        }
        print(f"{split}: {len(split_files[split])} files, {len(split_windows[split])} windows")
    ...
    print(f"\nSaved ESP-Fi HAR NPZ splits to: {output_dir}")
